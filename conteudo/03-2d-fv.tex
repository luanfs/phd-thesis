...
\mathbf{G}_{ij}\big[\mathbf{F}[{\overline{q},\tilde{c}^{y,n}}],\tilde{c}^{x,n}\big] &= 
   -\overline{q}\lambda \mathbf{G}_{ij}\big[ {\delta_i} \tilde{u}_{ij}^n,\tilde{c}^{x,n}\big] \\
...
However, if we compute the updated solution using Equation \eqref{chp-adv2d-AL}, we have that the error is given by
\begin{align}
...
\begin{align*}
&\mathbf{F}_{i}^E[{-\lambda \overline{q}\delta_j\tilde{v}^{n}_{j}(x),\tilde{u}^{n}}](y) =
-\lambda \overline{q}\mathbf{F}_{i}^E[{\delta_j\tilde{v}^{n}_{j}(x),\tilde{u}^{n}}](y)   \\
...
\bigg(\int_{t^n}^{t^{n+1}}
 [\Psi \big( x_{i+\frac{1}{2}}, y_{i,j+\frac{1}{2}}^d(\theta,t^{n+1}),\theta\big)-
  \Psi \big( x_{i-\frac{1}{2}}, y_{i,j+\frac{1}{2}}^d(\theta,t^{n+1}),\theta\big)]-
 [\Psi \big( x_{i+\frac{1}{2},j}^d, y_{i-\frac{1}{2},j}^d(\theta,t^{n+1}),,\theta\big)
 -\Psi \big( x_{i-\frac{1}{2},j}^d, y_{i-\frac{1}{2},j}^d(\theta,t^{n+1}),\theta\big)]
\,d\theta -\bigg)
\end{align*}

\begin{align*}
&\mathbf{G}_{j}^E[{-\lambda \overline{q}\delta_i\tilde{u}^{n}_{i}(y),\tilde{v}^{n}}](x) = 
 \mathbf{G}_{j}^E[{-\lambda \overline{q}\delta_i\tilde{u}^{n}_{i}(y),\tilde{v}^{n}}](x) \\ 
&= -\lambda^2\overline{q} \frac{1}{\Delta t} 
...
&= \frac{\overline{q}}{\Delta x^2} 
\bigg(\int_{t^n}^{t^{n+1}}
 [\Psi \big( x_{i+\frac{1}{2},j}^d(\theta,t^{n+1}),y_{j+\frac{1}{2}},\theta\big)-
 \Psi \big( x_{i+\frac{1}{2},j}^d(\theta,t^{n+1}),y_{j-\frac{1}{2}},\theta\big)]-
 [\Psi \big( x_{i-\frac{1}{2},j}^d(\theta,t^{n+1}),y_{i,j+\frac{1}{2}}^d,\theta\big)
 -\Psi \big( x_{i-\frac{1}{2},j}^d(\theta,t^{n+1}),y_{i,j-\frac{1}{2}}^d,\theta\big)]
\,d\theta -\bigg)
\end{align*}
