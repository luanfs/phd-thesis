...
%\mathcal{A}_{i+\frac{1}{2},j}^{x} = {c}_{i+\frac{1}{2},j}^{x,n} \Delta x \Delta y.
\end{align*}
This notation shall be useful when we consider these schemes on the cubed-sphere in Chapter \ref{chp-2d-fv}.
Hence, the operators $\mathbf{F}$ and $\mathbf{G}$ represent the numerical updates added
to solve the advection equation in the $x$ and $y$ directions
