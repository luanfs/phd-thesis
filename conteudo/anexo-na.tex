...
&= \frac{1}{h} \bigg(
 \int_{s}^{s+h}f(s+h_n,\theta) \,d\theta 
+\int_{s_0}^{s}f(s+h_n,\theta) \,d\theta 
-\int_{s_0}^{s}f(s,\theta) \,d\theta \bigg).
\end{align}
It follows from Theorem \ref{anexo-numint-mv} (with $g=1$) that there exists $\theta_n$ between $s$ and $s+h$ such that:
...
\begin{align*}
	\frac{1}{N^2}
	\sum_{i,j=1}^N \bigg(\partial^2_x f(\nu_i,y_j)+\partial^2_y f(\theta_i, \eta_j)\bigg) 
	= \partial^2_x f(\overline{x},\overline{y}) +\partial^2_y f(\overline{x},\overline{y}),
\end{align*}
