...
\begin{equation}
	...
	|\tilde{\Omega}_{ij}| + \mathcal{O}(\Delta x^2),
\end{equation}
where $|\tilde{\Omega}_{ij}| = \sqrt{g}(x_i,y_j) \Delta x \Delta y$,
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.5\linewidth]{equiangular_sphere}
...
The B duo-grid points are generated by computing the mapping $\boldsymbol{\Phi}$ for the grid-points
$(x_i,_y_j)$ for $i$ and $j$ out of the range $1, \ldots, N$.
When using the cube midpoint, the A, C and D-grid points are compute analogously.
In Figure we illustrate the duo-grid points obtained for both g0 and g2 grids.
