...
\end{equation}
where $|\hat{\Omega}_{ij}| = \sqrt{g}(x_i,y_j) \Delta x \Delta y$,
...
We refer to this grid points formulation as cube midpoints.
...
\end{align}
Hence, their midpoint approximations are define as:
\begin{align}
...
The B duo-grid points are generated by computing the mappings $\boldsymbol{\Phi}_p$ for the grid points $(x_i, y_j)$ where $i$ and $j$ range from $1$ to $N$. 
...
\begin{figure}[!htb]
	...
	\begin{subfigure}{0.45\textwidth}
		\includegraphics[width=0.8\linewidth]{g0_duo}
		\caption{g0 duo-grid.\label{cs-duo-g0}}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{g0_duo2}
		\caption{Modified g0 duo-grid.\label{cs-duo-g02}}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{g2_duo}
		\caption{g2 duo-grid.\label{cs-duo-g2}}
	\end{subfigure}
	\caption{Duo-grid lines of panel 1 for the g0 (a) and g2 (c). (b) shows the modified duo-grid of g0.
...
$\sqrt{g}(x,y)$ at time $t$: 
\begin{equation*}
	Q_{ijp}(t) = \frac{1}{|\Omega_{ij}|}\int_{x_{i-\frac{1}{2}}}^{x_{i+\frac{1}{2}}}
	\int_{y_{j-\frac{1}{2}}}^{y_{j+\frac{1}{2}}}  q(x,y,p,t) {\sqrt{g}(x,y)}\,dx \,dy.
\end{equation*}
...
We consider g0.s with dg1, g0.c with dg1, and g0.c with dg2, whose errors are depicted in Figure \ref{cs-duoscalar-tc1-g0}.
Additionally, we analyze g2.s with dg1, g2.c with dg1, and g2.c with dg2, and their errors are illustrated in Figure \ref{cs-duoscalar-tc1-g2}.

From the red and blue lines in Figure \ref{cs-tc1-error}, we observe that both dg1 and dg2 achieve fourth-order accuracy when the midpoints use the cube formulation, 
with dg2 being much more accurate than dg1.
However, when spherical midpoints are employed, we observe a reduction in accuracy by two orders, as indicated by the green lines.
...
Finally, we observe that the g0 grid yields slightly better results than g2.
\begin{figure}[!htb]
	\centering
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{g0_duo_error_tc-2_ha}
		\caption{Error for g0.\label{cs-duoscalar-tc1-g0}}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{g2_duo_error_tc-2_ha}
	...
	\caption{Error for the duo-grid interpolation of the scalar field of \eqref{duo-tc1} for the grid g0 (left) and g2 (right).
		Green lines use the dg1 method with g0.s and g2.s grids, while red lines use dg1 method with g0.c and g2.c grids,
		and blue lines use the dg2 method with g0.c and g2.c grids,\label{cs-tc1-error}}
\end{figure}
...
$\boldsymbol{u}:\mathbb{S}^2_R \to \mathbb{R}^3$. We also have its covariant components at the C and D grid points, namely $u_{i+\frac{1}{2},j,p}$ for $i=0, \ldots, N$ and $j=1, \ldots, N$, as well as
...
\begin{figure}[!htb]
	...
	\end{subfigure}
	\caption{Illustration of the C/D duo-grid interpolation for a scalar field. \label{cs-duovec}}
\end{figure}
...
\end{align*}
We apply a simple linear interpolation to remap $u$ and $v$ to the A-grid points (see Figure \ref{cs-duovec}).
...
to recover the wind at the ghost cell centers using any polynomial degree (see  Figures \ref{cs-duovec-3}).
Finally, we can use the values at the ghost cell centers to obtain the values at the ghost cell edges
by employing a linear interpolation once again.
...
from the A-duo-grid to duo-grid cell edges, the maximum attainable scheme order is 2.
...
Additionally, dg2 yields smaller errors, and overall, g0 performs better than g2.
\begin{figure}[!htb]
	\centering
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{g0_error_tc-2_uc}
		\caption{Error for g0.\label{cs-duoscalar-tc2-g0}}
	\end{subfigure}
	\begin{subfigure}{0.45\textwidth}
		\centering
		\includegraphics[width=0.8\linewidth]{g2_error_tc-2_uc}
		\caption{Error for g2.\label{cs-duoscalar-tc2-g2}}
	\end{subfigure}
	\caption{Error for the duo-grid interpolation of a C-grid wind given by Equation \eqref{duo-wind1} for the grid g0 (left) and g2 (right).
		Green lines use the dg1 method with g0.s and g2.s grids, while red lines use dg1 method with g0.c and g2.c grids,
		and blue lines use the dg2 method with g0.c and g2.c grids,\label{cs-tc2-error}}
\end{figure}
