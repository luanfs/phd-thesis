...
\begin{table}[!ht]
	\begin{tabular}{|c|l|l|}
		...
		IC2   & $0.5+0.5(1+\cos{\frac{\pi r}{r_0}})\chi_{r<r_0}(r)$, where $\chi_{r<r_0}(r)=1$ if $r<r0$, and $\chi_{r<r_0}(r)=0$ otherwise.  \\ \hline
		IC3   & $\exp(b_0[(X-X_1)^2+ (Y-Y_1)^2 + (Z-Z_1)^2]) + \exp(b_0[(X-X_2)^2+ (Y-Y_2)^2 + (Z-Z_2)^2])$ \\ \hline
	\end{tabular}
...
\begin{table}[!ht]
	\begin{tabular}{|c|l|l|l|l|}
		...
		& $-u_0\sin(\lambda)\sin(\alpha)$ & 3600 & 0.5 \\ \hline
		VF2   & $u_0\sin^2(\lambda_p)\sin(2\phi)\cos(\frac{\pi t}{T})+u_0\cos\phi$ 
		& $u_0\sin(2\lambda_p)\cos(\phi)\cos(\frac{\pi t}{T})$& 1600 &0.6 \\ \hline
		VF3   & $-u_0\sin^2(\frac{\lambda+\pi}{2})\sin(2\phi)\cos^2(\phi)\cos(\frac{\pi t}{T})$ 
		& $\frac{u_0}{2}\sin(\lambda+\pi)\cos^3(\phi)\cos(\frac{\pi t}{T})$ & 7200 &0.6 \\ \hline
	\end{tabular}
...
IC3 represents two Gaussian hills as suggested by \citet{nair:2010} and we set $b_0 = -5$.
IC2 represent a cosine bell centered at a cube corner, where $r_0 = \frac{R}{3}$, 
$r=2R \arcsin{\bigg(
\sqrt{\sin^2{\big(\frac{\phi-\phi_0}{2}\big)} + \cos{\phi}\cos{\phi_0}\cos^2{\big(\frac{\lambda-\lambda_0}{2}\big)}}\bigg)}$
is the geodesic distance,
for $\lambda_0=\frac{\pi}{4}$ and $\phi = \frac{\pi}{2}-\arccos{\frac{1}{\sqrt{3}}}$
The initial conditions are shown in Figure \ref{chp5-ic}, expect by IC2 which is very similar to IC1.
...
\newpage
\subsection{Advection of cosine bell hill through the rotated zonal wind}
...
\begin{figure}[!htb]
	...
	\end{subfigure}
	\caption{
		Advection experiment errors at a cube corner using IC2 and VF1 from Table \ref{chp5-ic} after 12 days, using hord8
		with PL (left) and LT schemes (right) on the g0 grid with $N=384$. 
...
\begin{figure}[!htb]
	...
	\caption{As Figure \ref{chp-advcs-sec-exp-adv2-linf} but using IC2 and VF1 from Table \ref{chp5-ic}.\label{chp-advcs-sec-exp-adv5-linf}}
\end{figure}
...
The third test case considers the divergence free wind VF2 from Table \ref{chp5-tab2}, along with the initial condition IC3 from Table \ref{chp5-tab1}, 
...
\begin{figure}[!htb]
	...
	\end{subfigure}
	\caption{Similar to Figure \ref{chp-advcs-sec-exp-adv2} but using IC3 and VF2 from Table \ref{chp5-ic}.\label{chp-advcs-sec-exp-adv3}}
\end{figure}
...
\begin{figure}[!htb]
	...
	\end{subfigure}
	\caption{
		Advection experiment errors using IC3 and VF2 from Table \ref{chp5-ic} after 12 days, using hord8
		with PL (left) and LT schemes (right) on the g0 grid with $N=384$. 
...
\begin{figure}[!htb]
	...
	\caption{As Figure \ref{chp-advcs-sec-exp-adv2-linf} but using  IC3 and VF2 from Table \ref{chp5-ic}.\label{chp-advcs-sec-exp-adv3-linf}}
\end{figure}
...
The fourth and last test case considers the divergent wind VF3 from Table \ref{chp5-tab2}, along with the initial condition IC3 from Table \ref{chp5-tab1}, 
where the velocity is time-dependent.
...
\begin{figure}[!htb]
	...
	\end{subfigure}
	\caption{Similar to Figure \ref{chp-advcs-sec-exp-adv2} but using IC3 and VF3 from Table \ref{chp5-ic}.\label{chp-advcs-sec-exp-adv4}}
\end{figure}
...
\begin{figure}[!htb]
	...
	\end{subfigure}
	\caption{
	 Advection experiment errors using IC3 and VF3 from Table \ref{chp5-ic} after 12 days, using hord8
	 with PL (left) and LT schemes (right) on the g0 grid with $N=384$.
...
\begin{figure}[!htb]
	...
	\caption{As Figure \ref{chp-advcs-sec-exp-adv2-linf} but using  IC3 and VF3 from Table \ref{chp5-ic}.\label{chp-advcs-sec-exp-adv4-linf}}
\end{figure}
