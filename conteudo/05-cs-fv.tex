...
\begin{figure}[!htb]
	...
	\caption{
		...
		the rotated zonal wind (VF1, Table \ref{chp5-vf}) after 12 days, using the limited PPM scheme (hord8) 
...
Blue lines indicate the use of the LT scheme, while green lines represent the PL scheme.
Solid lines represent the results with the unlimited PPM (hord0) scheme, whereas dashed lines represent the results with limited PPM (hord8).
...
\begin{figure}[!htb]
	...
	\caption{Advection experiment results using the cylinder at a cube corner (IC3, Table \ref{chp5-ic}) and 
		...
		We are using the LT-hord8-mf1 scheme on the equiangular grid (g2) with $N=192$. \label{chp-advcs-sec-exp-adv6}}
\end{figure}
...
\begin{figure}[!htb]
	...
	(e) depicts the reference solution. The limited PPM scheme is denoted by hord8.
...
\begin{figure}[!htb]
	...
	\caption{Advection experiment results using the two Gaussian hills  (IC4, Table \ref{chp5-ic}) and 
		...
		We are using the LT-hord8-mf1 scheme on the equiangular grid (g2) with $N=384$.\label{chp-advcs-sec-exp-adv3}}
\end{figure}
...
10  \eqref{chp-advcs-sec-exp-adv3-e},
and 12  \eqref{chp-advcs-sec-exp-adv3-f} days.
We are using the LT-hord8-mf1 scheme on the equiangular grid (g2) with $N=384$.
...
Once more, it is evident that all schemes with the unlimited PPM (hord0) achieve second-order accuracy as expected, while those with limited PPM (hord8) experience a reduced order 
...
We are using the LT-hord8-mf1 scheme on the equiangular grid (g2) with $N=384$. \label{chp-advcs-sec-exp-adv4}}
\end{figure}
...
\begin{figure}[!htb]
	...
	\end{subfigure}
	\caption{
		Advection experiment errors using the two Gaussian hills (IC4, Table \ref{chp5-ic}) and  the divergent wind (VF3, Table \ref{chp5-vf}) after 12 days, using the limited PPM scheme (hord8)
	 with PL (left) and LT schemes (right) on the equi-edge grid (g0) with $N=384$.
