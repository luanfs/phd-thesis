...
\begin{equation}
	...
	x_{i+\frac{1}{2}}^d(t^n,t^{n+1}) - \tilde{x}_{i+\frac{1}{2}}^n = O(\Delta t^p).
\end{equation}
...
This scheme will be referred to as \textbf{RK1}.
...
\begin{prop}
	...
	\begin{equation}
		x_{i+\frac{1}{2}}^d(t^n,t^{n+1}) - \tilde{x}_{i+\frac{1}{2}}^n = O(\Delta t^2),
	\end{equation}
...
the departure point, such as integrating the ODE from Equation \ref{chp-adv1d-sec-flux:prop1} using different
...
in the parabolas by replacing the term $\delta Q_i$ in Equation \eqref{chp-adv1d-sec-recon-ppm-eq7}
...
This scheme is referred to as \textbf{hord8}.
