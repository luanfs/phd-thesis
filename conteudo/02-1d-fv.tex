...
schemes \textbf{hord0} and  \textbf{hord8} which were presented in Section
\ref{chp-adv1d-sec-recon}, as well as the departure point schemes \textbf{dp1} and \textbf{dp2} described 
in Section \ref{chp-adv1d-sec-dp}. The code used in this section can be found in Appendix \ref{anexo-code}.
...
\begin{figure}[!htb]
	\centering
	\includegraphics[width=0.8\linewidth]{adv1d_tc1_N48_hord8_dp1_t13}
...
$N^{(k)} = 48 \times 2^k$,
$\Delta t^{(k)}=\frac{10000}{{2^{k}}}$, for $k = 0, \ldots, 4$.
...
We employed the PPM method introduced by \citet{colella:1984} (hord0) and its variant such as the one from \citet{lin:2004} (hord8).
