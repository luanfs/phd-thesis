...
The advection equation with periodic boundary conditions is given by the following PDE:
\begin{equation}
...
\subsection{Unlimited PPM reconstruction - hord0}
...
\textcolor{black}{
This name is justified because in FV3, the 1D advection solver input is named ``hord'' (horizontal advection method).
We note that hord0 is not implemented in FV3, but we implemented it because, since it has no limiter, it is suitable for checking order of accuracy for the schemes investigated in this work.
For a list of reconstruction schemes available in FV3, refer to \citet{harris:2021}. One may see that the scheme hord5 in FV3 is the closest to hord0, as hord5 only applies a filter on the hord0 flux to remove  2$\Delta x$ numerical noise}.

\subsection{Limited PPM reconstruction - hord8}
...
Finally, for both unlimited PPM (hord0) and limited PPM (hord8) schemes, $F_{i+\frac{1}{2}}^n$ uses the stencil
...
schemes unlimited PPM \textbf{hord0} (Subsection \ref{chp-adv1d-sec-hord0}) and limited PPM
...
\begin{figure}[!htb]
	...
	\caption{Linear advection experiment using the IC given by Equation \eqref{chp-adv1d-ic1} (black curve) with constant velocity.
		...
		Reconstruction schemes employed: unlimited PPM (hord0 - blue curve) and limited PPM (hord8 - orange curve).\label{chp-adv1d-sec-exp-adv1}}
\end{figure}
...
It is evident that the limited PPM (hord8) exhibit a significant advantage.
This scheme effectively prevent the strong oscillations observed in the unlimited PPM scheme (hord0),
...
\begin{figure}[!htb]
	...
	\caption{Linear advection experiment using the velocity from Equation \eqref{chp-adv1d-ic2},
		...
		We are using the limited PPM (hord8) scheme with the DP1 departure point scheme. \label{chp-adv1d-sec-exp-adv2}}
\end{figure}
\begin{figure}[!htb]
	\centering
	...
	\end{subfigure}
	\caption{Relative error for the unlimited PPM (hord0 -red lines) and limited PPM (hord8 - green lines) schemes
	         in $L_{\infty}$
...
for  the unlimited PPM (hord0), the DP1 scheme results in a first-order error in the departure point, which dominates the total error.
This observation is in agreement with the discussion in Section \ref{chp-adv1d-sec-dp}.
On the other hand, when employing the DP2 scheme, we can achieve third-order accuracy for the unlimited PPM (hord0).
For the limited PPM (hord8), the DP2 slightly reduces the $L_{\infty}$ error.

However, in the $L_1$ norm, as shown in Figure \ref{chp-adv1d-sec-exp-adv2-error-l1}, 
for both unlimited PPM (hord0) and limited PPM (hord8), we observe that DP1 results in a 1st order accuracy, 
...
without limiter (hord0) and its limited variant such as the one from \citet{lin:2004} (hord8).
...
error, despite the scheme having third-order accuracy in space. However, the DP2 scheme  with the unlimited PPM (hord0)
...
The DP2 scheme also showed to lead to a more accurate result when combined with limited PPM (hord8), especially in the $L_1$ norm.
