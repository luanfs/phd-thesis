...
def plot_grid(grid, map_projection):
    ...
    if map_projection == "mercator":
        ...
        #plt.figure(figsize=(1000/dpi, 1200/dpi), dpi=dpi)
        plt.figure(figsize=(800/dpi, 1600/dpi), dpi=dpi)
    elif map_projection == 'sphere':
        plateCr = ccrs.Orthographic(central_longitude=-135.0, central_latitude=0.0)
    ...
    for p in range(0, nbfaces):
        ...
        ng = 3
        if p>=0:
           for i in range(js, je, gap):
              for j in range(js, je+1, gap):
                 ax.plot([lonB[i,j], lonB[i+gap,j]],[latB[i,j], latB[i+gap,j]], '-', color='white', transform=ccrs.Geodetic())
    
           for i in range(js, je+1, gap	):
              for j in range(js, je, gap):
                 ax.plot([lonB[i,j], lonB[i,j+gap]],[latB[i,j], latB[i,j+gap]], '-', color='white', transform=ccrs.Geodetic())

        if p==-1:
           Color='gray'
           for j in range(14, 22):
              for i in range(3,6):
                 ax.plot(lonA[i,j], latA[i,j],marker='o',markersize=8,color=Color, transform=ccrs.Geodetic())

           for j in range(14, 22):
              for i in range(0,3):
                 ax.plot(lonA[i,j], latA[i,j],marker='o',markersize=8,color='gray', transform=ccrs.Geodetic())

           for j in range(14, 21):
              for i in range(0,6):
                 ax.plot([lonA[i,j], lonA[i,j+1]],[latA[i,j], latA[i,j+1]], '--', color='gray', transform=ccrs.Geodetic())

           for j in range(14, 22):
              for i in range(0,5):
                 ax.plot([lonA[i,j], lonA[i+1,j]],[latA[i,j], latA[i+1,j]], '--', color='gray', transform=ccrs.Geodetic())
        ...
        #         ax.plot(lonA[i,j], latA[i,j],marker='o',markersize=8,color='blue', transform=ccrs.Geodetic())
        if p==0:
           for j in range(js, je):
              for i in range(je-3, je):
                 ax.plot(lonA[i,j], latA[i,j],marker='o',markersize=4,color='blue', transform=ccrs.Geodetic())


        if p==1:
           for j in range(js-3, je+3):
              for i in range(js-3, je+3):
                 ax.plot(lonA[i,j], latA[i,j],marker='o',markersize=4,color='gray', transform=ccrs.Geodetic())
           for j in range(js-3, je+2):
              for i in range(js-3, je+3):
                 ax.plot([lonA[i,j], lonA[i,j+1]],[latA[i,j], latA[i,j+1]], '--', color='gray', transform=ccrs.Geodetic())
           for j in range(js-3,je+3):
              for i in range(js-3,je+2):
                 ax.plot([lonA[i,j], lonA[i+1,j]],[latA[i,j], latA[i+1,j]], '--', color='gray', transform=ccrs.Geodetic())
        ...
        if p==-1:
           Color='gray'
           for j in range(js-3, js+gap+ng):
              for i in range(js-3, js+gap+ng-1):
                 ax.plot([lonA[i,j], lonA[i+1,j]],[latA[i,j], latA[i+1,j]], '--', color=Color, transform=ccrs.Geodetic())

           for j in range(js-3, js+gap+ng-1):
              for i in range(js-3, js+gap+ng):
                 ax.plot([lonA[i,j], lonA[i,j+1]],[latA[i,j], latA[i,j+1]], '--', color=Color, transform=ccrs.Geodetic())

           for j in range(js-3, js+gap+ng):
              for i in range(js-3, js+gap+ng):
                 if(j<js and i<js):
                    ax.plot(lonA[i,j], latA[i,j], marker='o',markersize=5,color='black', transform=ccrs.Geodetic())
                 elif i<js and j>=js+gap:
                    ax.plot(lonA[i,j], latA[i,j], marker='o',markersize=5,color='orange', transform=ccrs.Geodetic())
                 elif i<js:
                    ax.plot(lonA[i,j], latA[i,j], marker='o',markersize=5,color='blue', transform=ccrs.Geodetic())
                 elif j<js and i<js+gap:
                    ax.plot(lonA[i,j], latA[i,j], marker='o',markersize=5,color='red', transform=ccrs.Geodetic())
                 elif j<js:
                    ax.plot(lonA[i,j], latA[i,j], marker='o',markersize=5,color='orange', transform=ccrs.Geodetic())
                 else:
                    ax.plot(lonA[i,j], latA[i,j], marker='o',markersize=5,color=Color, transform=ccrs.Geodetic())
 

        if p==-1:
           Color = 'gray'
           for j in range(js-3+gap, js+gap+gap+ng):
              for i in range(js-3, js+gap+ng):
                 ax.plot(lonA[i,j], latA[i,j], marker='o',markersize=5,color=Color, transform=ccrs.Geodetic())
           ...
           for j in range(js-3+gap, js+gap+gap+ng-1):
              for i in range(js-3, js+gap+ng):
                 ax.plot([lonA[i,j], lonA[i,j+1]],[latA[i,j], latA[i,j+1]], '--', color=Color, transform=ccrs.Geodetic())

           #for i in range(js, je):
           #   for j in range(js, je+2):
           #      ax.plot([lonA[i,j], lonA[i,j+1]],[latA[i,j], latA[i,j+1]], '--', color='blue', transform=ccrs.Geodetic())
              #ax.plot([lonA[i,je-1], lonD[i,je]],[latD[i,je-1], latC[i,je]], '--', color='blue', transform=ccrs.Geodetic())
        ...
        #if p==0:
           #for j in range(js, je):
           #   for i in range(js, js+3):
           #      ax.plot(lonA[i,j], latA[i,j], marker='o',markersize=5,color='green', transform=ccrs.Geodetic())
           ...
           #for j in range(js, je):
           #   for i in range(js, js+3):
           #      ax.plot([lonA[i,j], lonA[i+1,j]],[latA[i,j], latA[i+1,j]], '--', color='blue', transform=ccrs.Geodetic())
        #if p==4:
        #   for i in range(js, je):
        #      for j in range(je-3, je):
        #         ax.plot([lonA[i,j], lonA[i+1,j]],[latA[i,j], latA[i+1,j]], '--', color='blue', transform=ccrs.Geodetic())
        ...
        plt.ylim(-45,45)
        plt.xlim(30,60)
        #ax.plot([B_lon, C_lon], [B_lat, C_lat], '-', linewidth=2*lw, color='red', transform=ccrs.Geodetic())
        # Plot A grid
        #if p==0:
        #  colora = 'black'
        #  markera='o'
        ...
        #   for j in range(js, je):
        #      ax.plot(lonA[i,j], latA[i,j], marker='s',color = colora, transform=ccrs.Geodetic())
       
        #if p==0: # plot east A grid ghost cells
        #   for g in range(0,3):
        #      i = ie+g
        #      for j in range(js-g, je+g):
        #          ax.plot(lonA[i,j], latA[i,j], marker='o',color = 'blue', transform=ccrs.Geodetic())
        #      ax.plot(lonA[i,je+g], latA[i,je+g], marker='o',color = 'magenta', transform=ccrs.Geodetic())
        #     ax.plot(lonA[i,js-1-g], latA[i,js-1-g], marker='o',color = 'magenta', transform=ccrs.Geodetic())

        
        #for i in range(IS, ie+1):
        #   for j in range(js, je+1):
        #      ax.plot(lonB[i,j], latB[i,j], marker='s',markersize=3,color = colora, transform=ccrs.Geodetic())

        #if p==0: # plot east B grid ghost cells
        #   for g in range(0,3):
        #      i = ie+1+g
        #      for j in range(js-g, je+1+g):
        #          ax.plot(lonB[i,j], latB[i,j], marker='o',color = 'blue', transform=ccrs.Geodetic())
        #      ax.plot(lonB[i,je+1+g], latB[i,je+1+g], marker='o',color = 'magenta', transform=ccrs.Geodetic())
        #      ax.plot(lonB[i,js-1-g], latB[i,js-1-g], marker='o',color = 'magenta', transform=ccrs.Geodetic())

       # plt.xlim((0, 90)) 
       # for i in range(IS, ie):
       #    for j in range(js, je):
       #       ax.plot(lonA[i,j], latA[i,j], marker='o',color = 'black', transform=ccrs.Geodetic())
        #if p == 0:
        #    # Plot B grid
        #    for i in range(isd, ied+1):
        #        for j in range(jsd, jed+1):
        #            ax.plot(lonB[i,j], latB[i,j], marker='o',color = 'white', transform=ccrs.Geodetic())


            # Plot A grid
            #for i in range(isd, ied):
            #    for j in range(jsd, jed):
            #        ax.plot(lonA[i,j], latA[i,j], marker='D',color = 'white', transform=ccrs.Geodetic())

            # Plot C grid
            #for i in range(IS, ie+1):
            #    for j in range(js, je):
            #        ax.plot(lonC[i,j], latC[i,j], marker='+',color = 'blue', transform=ccrs.Geodetic())

            # Plot D grid
            #for i in range(IS, ie):
            #    for j in range(js, je+1):
            #        ax.plot(lonD[i,j], latD[i,j], marker='+',color = 'red', transform=ccrs.Geodetic())
